...
def plot_step(t, y, title="Sprungantwort", save=None, show=True, 
              ...
              show_input=False, u_amplitude=1.0, u_signal=None):
    """
    ...
    if show_input:
        if u_signal is not None:
            ...
            u[t < 0] = 0
            ax.plot(t, u, 'r--', linewidth=2, alpha=0.7, 
    ...
    # Endwert-Linie
    steady_state = y[-1]
    ax.axhline(y=steady_state, color='olive', linestyle='--', 
...
def plot_step_with_metrics(t, y, title="Sprungantwort mit Metriken", save=None, show=True):
    """
    ...
    if settling_time < t[-1]:
        ax.axvline(x=settling_time, color='green', linestyle='--', 
                   linewidth=1.5, alpha=0.7, label=f'Ausregelzeit: {settling_time:.2f}s')
    ...
    if overshoot_abs > tolerance:  # Nur wenn signifikant
        ...
        ax.annotate(f'Überschwingen: {overshoot_pct:.1f}%', 
